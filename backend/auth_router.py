...
from backend import features, models
...
def me(
    ...
    return {
        ...
        # 수강 변경 탐색을 열어도 되는지 + 어느 학기 것인지. **시각 판정은 서버가
        # 합니다** — 기기 시계로 재면 시계가 틀어진 폰에서 마감이 지나도 열려 있습니다.
        # 값은 관리자 화면에서 고칩니다 (`backend/features.py`)
        "trade": features.trade_config(db),
