...
```json
{
  ...
  "is_demo": false,
  "trade": { "open": true, "year": 2026, "semester": 2 },
...
**`trade`는 수강 변경 탐색(Trade)의 노출 조건**입니다. 홈의 배너, 하단 네비·사이드바
메뉴, `/trade` 라우트, 홈의 `[기존 시간표 | 트레이드 계획]` 전환이 **전부 이 하나로
갈립니다.** 값은 `settings` 표에 있고 `/admin/features/trade` 로 고칩니다.

`open` 은 **스위치와 마감만** 본 결과입니다. `year`/`semester` 를 같이 보내는 이유는
**서버가 그 사람이 어느 학기를 열어 두었는지 모르기** 때문입니다 — 화면이 지금 보고 있는
학기와 대 봐서 최종 판단합니다.

⚠️ **판정을 화면에 맡기지 마세요.** 브라우저의 `Date.now()` 는 그 기기가 믿는 시각이라,
시계가 틀어진 폰에서는 마감이 지나도 열려 있습니다. 마감이 있는 기능은 **한 시계**로
재야 하고 그 시계는 서버입니다. 화면 쪽 `lib/features.ts` 에는 학기 조건만 남아 있습니다.
...
```json
{ "open": true, "enabled": true, "year": 2026, "semester": 2,
  "until": "2026-08-28T08:00:00+00:00" }
```

`PATCH` 는 **준 칸만** 바꿉니다 — 스위치만 내리려고 마감까지 다시 보내면 그 사이 다른
자리에서 바꾼 값을 덮어씁니다. `until` 이 `null` 이면 기한 없음(스위치로만 여닫음)이고,
읽을 수 없는 문자열이면 `422` 입니다 — 저장해 버리면 그 뒤로 마감이 조용히 사라집니다.

⚠️ **`until` 에는 시간대를 붙이세요.** 저장이 문자열이라 기준이 문자열 안에 있어야
어디서 읽든 같은 순간입니다. 시간대가 없으면 UTC 로 읽습니다.

⚠️ **`settings` 표가 비어 있으면 코드의 기본값을 씁니다**(`backend/features.py`).
값을 지우는 것이 곧 "기본으로 되돌리기" 입니다.
