...
| `home/TodayCardV3` | 홈 배치 **V3 — 배포본**. **머리 한 줄**(지금 \| Events \| Meal) + **본문 두 칸**(주간 격자 \| 오늘). `WeekTimetable` 을 **자기가 그립니다** — 오늘과 나란히 놓아야 해서 `HomePage` 아래에 따로 붙이면 어긋납니다. DOM 은 오늘이 먼저고 `xl:order-2` 로 넓을 때만 자리를 바꿉니다(좁으면 한 주가 오늘 위로 올라갑니다). 급식은 **머리 줄**입니다 — 아래로 내리면 자·목록 사이에 끼어 흐름이 끊기고, `meal` 이 `null` 이어도 칸을 남겨 "급식 정보가 없습니다" 를 띄웁니다. 히어로는 목록이 못 하는 말만 합니다 — 남은 시간(진행바)·다음 수업·**교실 이동**(`current.room !== next.room`), 그리고 **연강 단위**(`currentPeriods`)로 잡아야 20:20 에 끝난다면서 NEXT 에 같은 과목을 또 거는 일이 없습니다. ⚠️ **수업이 없는 날(주말·방학)에는 오늘 칸의 카드를 아예 안 그립니다** — 조건을 `RetroCard` 안에 두면 테두리와 그림자만 남아 **검은 줄 하나**가 화면에 박힙니다. 넓은 화면에서 자리(1fr)를 지켜야 옆의 주간 격자가 27rem 을 유지하므로 **바깥 칸은 남기고** 좁은 화면에서만 `hidden xl:flex` 로 간격까지 걷습니다. `planned` 를 켜면 날짜 옆에 `계획 미리보기` 표식이 붙고 `WeekTimetable` 에도 그대로 넘깁니다 — 계획은 아직 등록된 시간표가 아니라서 어느 쪽을 보는지 헷갈리면 엉뚱한 교실로 갑니다 |
