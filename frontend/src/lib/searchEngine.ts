 ...
 */
export const evaluateBoolExpression = (expression: string, pool: string[]): boolean => {
    const trimmedExpr = expression.trim();
    if (!trimmedExpr) return true;
    
    const normalizedPool = pool.map(p => String(p).toLowerCase());

    const tokens = trimmedExpr.match(/\(|\)|&&|&|\+|[^\(\)+&]+/g)?.map(t => t.trim()).filter(t => t) || [];
...
const parseQuery = (searchTerm: string) => {
    ...
    const flatTerms = matchBase.split(/[+&()\/]+/).map(t => t.trim().toLowerCase()).filter(t => t);
 ...
 */
const filterMatchingClasses = (allData: any[], queryParams: ReturnType<typeof parseQuery>, selectedYears: string[]) => {
    const { mode, effectiveQuery, subjectExpr, personExpr, isStrictMode, flatTerms } = queryParams;
    ...
    allData.forEach(subject => {
        ...
        const isSubjectMatch = personExpr 
            ? (evaluateBoolExpression(subjectExpr, [subjectName]) && evaluateBoolExpression(personExpr, Array.from(subjectPersonPool)))
            : evaluateBoolExpression(effectiveQuery, [subjectName, ...Array.from(subjectPersonPool)]);
        ...
        subject.sections.forEach((sec: any) => {
            // 필터링된 학생 목록 생성
            const activeStudents = sec.students.filter((s: any) => selectedYears.includes(s.stuId.split("-")[0]));
            ...
            const sectionInfoPool = [subjectName, sec.section, sec.teacher, sec.room];
            ...
            if (personExpr !== null) {
                ...
                const pM = evaluateBoolExpression(personExpr, sectionPersonPool);
                isSectionMatch = sM && pM;
            } else if (mode === "student") {
                // student: 인 경우: 해당 인물이 이 분반에 있는지 확인
                isSectionMatch = evaluateBoolExpression(effectiveQuery, sectionPersonPool);
            } else if (mode === "teacher") {
                // teacher: 인 경우: 해당 교사의 분반인지 확인
                isSectionMatch = evaluateBoolExpression(effectiveQuery, [sec.teacher]);
            } else if (isStrictMode) {
                // && 연산자가 포함된 일반 검색
                isSectionMatch = evaluateBoolExpression(effectiveQuery, [...sectionInfoPool, ...sectionPersonPool]);
            } else {
                // 일반 Soft mode 검색 (mode === "general")
                if (flatTerms.length === 0) {
                    isSectionMatch = activeStudents.length > 0;
                } else {
                    const isInfoMatch = evaluateBoolExpression(effectiveQuery, sectionInfoPool);
                    const isPersonMatch = flatTerms.some(term => sectionPersonPool.some(p => p.toLowerCase().includes(term)));
...
const extractEntities = (matchingClasses: any[], flatTerms: string[], selectedYears: string[]) => {
    ...
    matchingClasses.forEach(cls => {
        if (flatTerms.some(t => cls.teacher.toLowerCase().includes(t))) {
            ...
            if (!entityMap.has(key)) {
                entityMap.set(key, { type: "teacher", name: cls.teacher, id: "Teacher", subjects: new Set() });
            }
            entityMap.get(key).subjects.add(cls.subject);
        }

        cls.students.forEach((s: any) => {
            if (flatTerms.some(t => s.stuId.toLowerCase().includes(t) || s.name.toLowerCase().includes(t))) {
                if (!entityMap.has(s.stuId)) {
                    entityMap.set(s.stuId, { type: "student", name: s.name, id: s.stuId, subjects: new Set() });
                }
...
export const searchInClient = (allData: any[], searchTerm: string, selectedYears: string[]): SearchResult => {
    if (!searchTerm.trim()) {
        const filteredData = allData.map(subject => ({
            ...subject,
            sections: subject.sections.filter((sec: any) =>
                sec.students.some((s: any) => selectedYears.includes(s.stuId.split("-")[0]))
            ).map((sec: any) => ({
                ...sec,
                students: sec.students.filter((s: any) => selectedYears.includes(s.stuId.split("-")[0]))
            }))
        })).filter(subject => subject.sections.length > 0);
        
        return { 
            data: filteredData, entities: [], mode: "general",
            stats: { keyword: "", total_subjects: filteredData.length, total_sections: filteredData.reduce((acc, s) => acc + s.sections.length, 0) }
        };
    }

    const queryParams = parseQuery(searchTerm);
    const matchingClasses = filterMatchingClasses(allData, queryParams, selectedYears);
    const entities = extractEntities(matchingClasses, queryParams.flatTerms, selectedYears);
    ...
    const finalData = Object.keys(grouped).sort().map(sub => {
        const secs = grouped[sub].sort((a, b) => {
            const aNum = parseInt(a.section.match(/\d+/)?.[0] || "0");
            const bNum = parseInt(b.section.match(/\d+/)?.[0] || "0");
            return aNum - bNum;
        });
        const subStus = new Set(secs.flatMap(s => s.students.map((st: any) => st.stuId)));
        return { subject: sub, subject_student_count: subStus.size, section_count: secs.length, sections: secs };
    });
